...
describe("円形カラーピッカー", () => {
  it("背景・柄・文字で共通利用する色選択部品に円形UI、明るさ、HEX入力、お気に入り登録がある", () => {
    ...
    expect(source).toContain("<CircularColorPicker label={label} value={value} favoriteLabel={favoriteLabel} favoriteColors={favoriteColors}");
    expect(source).toContain('aria-label={`${favoriteLabel}をお気に入りに登録`}');
    expect(source).toContain('"☆ 登録"');
