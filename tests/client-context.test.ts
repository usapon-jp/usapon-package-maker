...
describe("ブラウザ環境判定", () => {
  it("iPhoneのInstagram内ブラウザを判定する", () => {
    ...
    expect(context).toEqual({ isIPhone: true, isAndroid: false, isInstagramInAppBrowser: true, shouldRecommendSafari: true });
  });
  ...
  it("iPhone SafariをInstagram内ブラウザとして扱わない", () => {
    ...
    expect(context).toEqual({ isIPhone: true, isAndroid: false, isInstagramInAppBrowser: false, shouldRecommendSafari: false });
  });
  ...
  it("デスクトップブラウザをiPhoneとして扱わない", () => {
    ...
    expect(context).toEqual({ isIPhone: false, isAndroid: false, isInstagramInAppBrowser: false, shouldRecommendSafari: false });
  });
  ...
  it("AndroidのInstagramにはSafari案内を出さない", () => {
    ...
    expect(context).toEqual({ isIPhone: false, isAndroid: true, isInstagramInAppBrowser: true, shouldRecommendSafari: false });
  });
});
